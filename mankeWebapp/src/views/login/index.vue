<template>
  <van-form @submit="onSubmit" style="margin-top: 30px">
    <van-field v-model="formdata.username" name="用户名" label="用户名" placeholder="用户名"
      :rules="[{ required: true, message: '请填写用户名' }]" style="padding-left: 30px; font-size: 16px" />
    <van-field v-model="formdata.password" type="password" name="密码" label="密码" placeholder="密码"
      :rules="[{ required: true, message: '请填写密码' }]" style="padding-left: 30px; font-size: 16px" />
    <van-field v-model="formdata.vcode" name="验证码" label="验证码" placeholder="验证码"
      :rules="[{ required: true, message: '请填写验证码' }]" style="padding-left: 30px; font-size: 16px" />
